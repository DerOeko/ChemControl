function [out] = ChemControl_mod83_modSim(parameters, subj)
    % Standard Q-learning model with delta learning rule.
    
    % ----------------------------------------------------------------------- %
    % Unpack parameters
    ep_act = sigmoid(parameters(1));
    ep_spec = sigmoid(parameters(2));
    rho = exp(parameters(3));
    goBias = parameters(4);
    ...
    for t = 1:T
        ...
        v_pe = p_g - sv(s);
        sv(s) = sv(s) + ep_spec * v_pe;

        if a==1
            loglik = loglik + log(p1 + eps);

            q_pe = o-q_g(s)/rho;
            q_g(s) = q_g(s) + ep_act * (rho * o - q_g(s));
        elseif a==2
            loglik = loglik + log((1-p1) + eps);

            q_pe = o-q_ng(s)/rho;
            q_ng(s) = q_ng(s) + ep_act * (rho * o - q_ng(s));
        end
