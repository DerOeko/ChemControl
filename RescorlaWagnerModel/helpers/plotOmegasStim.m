function figHandle = plotOmegasStim(averageOmegasCell, cProbs, modelsWithOmega, figHandle)
arguments
    averageOmegasCell;
    cProbs;
    modelsWithOmega = [];
...
for iModel = 1:nModels
    ...
    if ~isempty(averageOmegas)
        nStates = size(averageOmegas, 2);

        % Plot each state separately
        for state = 1:nStates
            plot(1:length(averageOmegas(:, state)), averageOmegas(:, state), ...
                 'LineWidth', 2, 'DisplayName', states(state));
        end
        ylabel('Omega value');
