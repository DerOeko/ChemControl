function [loglik] = ChemControl_mod80(parameters, subj)
    % Unpack parameters
    ...
    for b = 1:B
        for t = 1:T
            ...
            if a==1
                PE_actor = o - Q_actor_g(s);

                loglik = loglik + log(p_action + eps);
                Q_actor_g(s) = Q_actor_g(s) + ep * (o - Q_actor_g(s));
            else
                PE_actor = o - Q_actor_ng(s);

                loglik = loglik + log(p_noaction + eps);
                Q_actor_ng(s) = Q_actor_ng(s) + ep * (o - Q_actor_ng(s));
            end
