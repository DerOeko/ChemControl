function [loglik] = ChemControl_mod86(parameters, subj)
    % Unpack parameters
    ...
    for b = 1:B
        for t = 1:T
            ...
            Q_combined_ng(s) = omega * Q_actor_ng(s) + (1-omega) * -V_spectator(s);
            ...
            Omega = Omega + (alpha*p_explore) * (abs(PE_spectator) - abs(PE_actor) - Omega);
            omega = 1/(1+exp(-beta*(Omega-thres)));
        end
    end
end
