function [loglik] = ChemControl_mod60(parameters,subj)
...
for b = 1:B
    ...
    for t=1:T
        ...
        v_pe = o - sv(s)/rho;
        sv(s) = sv(s) + ep * (1-omega(s)) * (rho * o - sv(s));

        if a==1
            loglik = loglik + log(p1 + eps);
            q_pe = o-q_g(s)/rho;

            q_g(s) = q_g(s) + ep *omega(s)* (rho * o - q_g(s));
            p_explore = p2;
        ...
        elseif a==2
            ...
            q_pe = o-q_ng(s)/rho;
            q_ng(s) = q_ng(s) + ep*omega(s) * (rho * o - q_ng(s));
            p_explore = p1;
        end
