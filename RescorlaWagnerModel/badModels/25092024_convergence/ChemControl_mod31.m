function [loglik] = ChemControl_mod31(parameters,subj)

% Dynamic Omega, dynamic pavlov, without pavlov on the nogo side,
% p_explore, arr, randexp by omega, double weighting
...
for b = 1:B
    ...
    for t=1:T
        ...
        w_g(s) = omega*q_g(s) + goBias + (1-omega)*pi*sv(s);
        w_ng(s) = omega*q_ng(s) + (1-omega)*pi*(-sv(s));
